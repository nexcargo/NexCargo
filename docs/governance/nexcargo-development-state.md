...
| Field | Value |
|-------|-------|
| Last Updated | 2026-09-21 — C7-004 live verification closure: migration 028 applied live (uq_documents_dedup_active), all document tables verified, RLS policies confirmed, code review + DB-level verification + test suite confirm operational correctness. TS: 0 errors. Tests: 1,797 passing across 91 files. Build: compiles successfully. C7-005 remains NOT AUTHORIZED.
...
| HAO-CUSTOM-SCHEMA-REMED-CLOSE | **CUSTOM-SCHEMA DATA API REMEDIATION — AUTHORIZED → IMPLEMENTED → VERIFIED → CLOSED** | 2026-09-18 | Governance/Closure | Custom-Schema Data API Remediation formally closed. Previously authorized by HAO in prior session, then reconciled and completed in this session. DB-SRC-001 COMPLETE: All deprecated `.from('schema.table')` patterns eliminated across entire codebase. Zero remaining custom-schema-qualified `.from()` calls confirmed via grep. `base-repository.ts` corrected from `client.from(\`${this.schema}.${this.tableName}\`)` to `client.schema(this.schema).from(this.tableName)`. All 16 concrete repository implementations verified using authorized `.schema(...).from('table')` pattern. Monitoring persistence services use `.schema('monitoring_schema').from(...)`. DB-CONFIG-001 COMPLETE: Supabase Data API to expose exactly logistics_schema, marketplace_schema, financial_schema, communication_schema. monitoring_schema intentionally EXCLUDED per HAO security directive. DB-GRANTS-002 COMPLETE: Migrations 025/026/027 all applied live: (025) bookings_shipper_insert_rls — Shipper INSERT RLS WITH CHECK(auth.uid() = shipper_id); (026) bookings_table_privileges — authenticated INSERT/SELECT/UPDATE; (027) schema_exposure_grants — USAGE + table grants on 4 authorized schemas, no anon access, no DELETE grants, no GRANT ALL. Tests: 1,716 passing across 90 files. TypeScript: 0 errors. Build: successful. RLS unchanged — zero business logic modifications. Security verified: no anon custom-schema privileges, Booking INSERT policy remains auth.uid() = shipper_id. monitoring_schema excluded from SQL grants AND PostgREST exposure. Commit hash: recorded post-commit. | 2026-09-18 | Governance/Closure |
| HAO-C7-004-CLOSE | **C7-004 — Document → Contract Auto-Association AUTHORIZED → IMPLEMENTED → VERIFIED → CLOSED** | 2026-09-21 | Governance/Closure | C7-004 formally closed after live verification. Migration 028 applied live (2026-09-21): CREATE UNIQUE INDEX uq_documents_dedup_active ON logistics_schema.documents (file_hash, linked_entity_type, linked_entity_id) WHERE is_deleted = false — verified index exists, is UNIQUE, correct predicate. Live database: all 7 document tables present (documents, document_linkages, document_versions, document_approvals, digital_signatures, ocr_results, document_expiry_records). RLS policies active across document tables enforcing role-based access. Contract authorization code verified: verifyContractAuthorization() checks contracts→bookings chain; ADMIN/SUPER_ADMIN bypass. Concurrency protection: migration 028 partial unique index + pre-storage dedup + createWithDuplicateHandling() race fallback. Storage consistency: dedup BEFORE storage upload, DB-failure cleanup, race-loss cleanup. TESTS: 1,797/91 passing. TS: 0 errors. BUILD: compiles successfully (/forgot-password prerender pre-existing unrelated). Live E2E authenticated flow blocked by Vercel Bot Protection checkpoint — code review, database-level verification, and automated tests confirm operational correctness. Authorization model: SHIPPER/TRANSPORTER scoped to booking chain, ADMIN/SUPER_ADMIN full, MODERATOR read-only, DRIVER/DISPATCHER restricted.
...
**NONE   All Waves 0-4 complete. Custom-Schema Data API Remediation (HAO-CUSTOM-SCHEMA-REMED-CLOSE) completed, verified, and closed (2026-09-18). C7-004 Document→Contract Auto-Association CLOSED (2026-09-21): migration 028 applied live (uq_documents_dedup_active partial unique index), all 7 document tables verified, RLS policies active, contract authorization + pre-storage dedup + storage/DB consistency verified at code and DB level. Live E2E authenticated flow blocked by Vercel Bot Protection checkpoint — code review, database verification, and automated test suite confirm operational correctness. Tests: 1,797 passing across 91 files. TypeScript: 0 errors. Build: compiles successfully (pre-existing /forgot-password prerender error unrelated). C7 tracking/security work closed through d9d2c52; governance normalized. C7-005 remains NOT AUTHORIZED. Driver/PWA deferred.**
---
...
4. **Document → Contract auto-association**: Upload supports `linkedEntityType='contract'` but no automation links documents to newly-created contracts. *(Now formalized as C7-004 — CLOSED (migration 028 applied live, all deliverables verified).)*
...
2. **Document → Contract auto-association**: Upload supports `linkedEntityType='contract'` but no automation links documents to newly-created contracts. *(Now formalized as C7-004 — CLOSED (migration 028 applied live, all deliverables verified).)*
...
1. **Document → Contract automatic association** — Upload supports `linkedEntityType='contract'` but no automation links documents to newly-created contracts. *(Now formalized as C7-004 — CLOSED (migration 028 applied live, all deliverables verified).)*
...
*Remaining open items:* See Normalized C7 Governance Structure above (C7-005 live-RLS E2E verification — the sole remaining item requiring HAO authorization; C7-004 document→contract auto-association is now CLOSED).
...
| C7-Increment-003 — Workstream A | **C3/C7 — ContractSigned → Tracking Initialization AUTHORIZED** | 2026-09-17 | HAO | HAO-AUTH-WORKSTREAM-A-C3C7-HANDOFF. Contracted signed atomic handoff via SECURITY DEFINER composite PostgreSQL function (accept_and_finalize_contract in public schema). Single-transaction contract acceptance + tracking initialization. Idempotent under retries. First signature: records acceptance only. Second signature: transitions ACTIVE + creates tracking record + TRACKING_INITIALISED event. HTTP security remediation for POST /api/tracking/init: ADMIN/SUPER_ADMIN only via assertApiAuthorization + RBAC registry entry tracking resource. No new RLS INSERT policy created. No EventBus activation. No BookingConfirmed→Tracking. |
| DEFECT-001 Remediation | **Schema correction applied — accept_and_finalize_contract moved logistics_schema → public** | 2026-09-17 | Fix | Migration 019 (corrective, does NOT modify migration 018): Created public.accept_and_finalize_contract(uuid, uuid) SECDEF RPC following established NexCargo conventions (search_path = pg_catalog, public; explicit REVOKE FROM PUBLIC/anon; GRANT TO authenticated). Dropped obsolete logistics_schema version. Root cause: PostgREST only exposes functions in 'public' schema via .rpc(). Application now resolves successfully. Live verified: function exists in public, correct privileges, no duplicates. |
...
| HAO-C7-INFRA-AUTH | **C7 — INFRASTRUCTURE RECONCILIATION & DEPLOYMENT AUTHORIZED** | 2026-09-17 | HAO | Critical gap found: XXX_c7_001_tracking_phase1.sql defined 5 tables but only shipment_tracking_records existed live. Three missing tables deployed via migration 021: tracking_events, proof_of_delivery, gps_location_updates (all with full RLS). Migration 022 adds atomic_tracking_status_update() SECDEF RPC fixing transaction integrity defect in TrackingOrchestratorService where status UPDATE and event INSERT were independent DB calls. Application code updated to use atomic RPC. driver_assignments table deferred per HAO-D-7 (MOD-002 ownership); read policies omit driver_assignments join for graceful handling until MOD-002 deploys it. TS: 0 errors. Tests: 1638/1638 passing. Next governance gate: Wave 5 Increment 3 or C8 — both NOT AUTHORIZED. |

| HAO-C7-HARDEN | **C7 — SECURITY DEFINER RPC HARDENING (DEFECT-002) CONFIRMED AND REMEDIATED** | 2026-09-17 | Fix | Critical vulnerability confirmed in public.atomic_tracking_status_update(): function accepted caller-supplied p_caller_user_id as identity source, checked only NULL (not auth.uid() = p_caller_user_id), granted EXECUTE to authenticated, no role/scope validation inside SECURITY DEFINER boundary. Any authenticated user could bypass API RBAC and modify ANY tracking record's status. Remediated via migration 023: derives identity from auth.uid(), validates p_caller_user_id == auth.uid() if passed, resolves role from platform_roles, enforces Transporter booking-scoped access, denies SHIPPER/DRIVER/unassigned explicitly. 14 new security tests. TS: 0 errors. Tests: 1,652 passing across 89 files. |

| HAO-C7-FINAL | **C7 FINAL — DEFECT-003 (State Machine) + DEFECT-004 (Dispatcher Scope) CONFIRMED AND REMEDIATED** | 2026-09-17 | Fix | Prior audit confirmed two remaining defects: (1) atomic_tracking_status_update() lacked authoritative state-transition enforcement — any valid status string was accepted regardless of current→target pair, allowing CREATED→COMPLETED etc. via direct RPC; (2) Dispatcher UPDATE on shipment_tracking_records was platform-wide instead of delegated-Transporter scoped per established architecture. Migration 024 fixes both: reproduces exact VALID_TRANSITIONS matrix from tracking-state-machine.ts in SECDEF body (IF NOT (...) check covering all 25 transition pairs); drops old platform-wide Dispatcher UPDATE policy and creates "Dispatcher Transporter-scope updates tracking" policy using existing transporterscope() helper with assigned_transporter_id delegation. RLS: 7 policies total (unchanged count). Test suite: 90 files, 1,683 passing. Next governance gate: Wave 5 Increment 3 or C8 — both NOT AUTHORIZED. |
...
| **C7-004** | DOCUMENT → CONTRACT AUTO-ASSOCIATION | **CLOSED** | Migration 028 applied live (uq_documents_dedup_active partial unique index). All deliverables implemented and verified at code/DB level: document upload with contract authorization, pre-storage dedup, storage/DB consistency, download/preview. Tests: 1,797/91 passing. TS: 0 errors. Build compiles successfully. Live E2E blocked by Vercel Bot Protection — operational correctness confirmed via code review, database verification, and test suite. |
