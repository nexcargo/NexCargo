...
| HAO-C7-004-CLOSE | **C7-004 — Document → Contract Auto-Association AUTHORIZED → IMPLEMENTED → VERIFIED → CLOSED** | 2026-09-18 | Governance/Closure | C7-004 corrective completion formally closed. Commit 04ca816 on main. DELIVERABLES: (1) POST /api/documents/upload — complete operational upload with contract authorization through contracts→bookings chain (shipper_id/transporter_id proof), file validation, SHA-256 hashing, storage upload, deduplication check, atomic document+linkage+version creation, orphaned-file cleanup on DB failure. (2) GET /api/documents — list endpoint supports linkedEntityType/linkedEntityId filtering plus backward-compatible entityType/entityId aliases, status/documentType pagination. (3) GET /api/documents/[id]/download — signed URL redirect with specific document access verification (creator ownership, admin bypass, shipper/transporter entity-chain check). (4) GET /api/documents/[id]/preview — same authorization as download for preview URLs. (5) DocumentRepository.createWithDuplicateHandling() — concurrency-safe insert with unique constraint violation recovery. (6) ShipmentDocumentViewer UI — fixed API parameter mismatch (entityType→linkedEntityType). AUTHORIZATION MODEL: SHIPPER may associate documents only with contracts in their booking scope; TRANSPORTER same; ADMIN/SUPER_ADMIN full access; MODERATOR read-only; DRIVER/DISPATCHER restricted. CONCURRENCY: pre-check dedup findExistingByHashAndEntity() + DB-level duplicate key violation handling in createWithDuplicateHandling(). STORAGE CONSISTENCY: storage path tracked; if DB persistence fails, deleteFile() cleans up orphaned object. TESTS: 1,797 passing across 91 files. TS: 0 errors. BUILD: compiles successfully (pre-existing /forgot-password prerender error unrelated). | 2026-09-18 | Governance/Closure |
...
**NONE   All Waves 0-4 complete. Custom-Schema Data API Remediation (HAO-CUSTOM-SCHEMA-REMED-CLOSE) completed, verified, and closed (2026-09-18). C7-004 Document→Contract Auto-Association formally closed (2026-09-18, commit 04ca816): complete operational upload with contract authorization through contracts→bookings chain, concurrency-safe deduplication, storage/DB consistency with orphan cleanup, download/preview with specific document access verification. Tests: 1,797 passing across 91 files. TypeScript: 0 errors. Build: compiles successfully. C7 tracking/security work closed through d9d2c52; governance normalized. C7-005 remains NOT AUTHORIZED. Driver/PWA deferred.**
