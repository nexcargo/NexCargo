...
| HAO-C7-004-WIP | **C7-004 — Document → Contract Auto-Association AUTHORIZED → IMPLEMENTED — AWAITING LIVE VERIFICATION** | 2026-09-18 | Governance/In-Progress | C7-004 implementation COMPLETE and committed (38e386d + bade642) but AWAITING LIVE VERIFICATION.  DELIVERABLES: (1) POST /api/documents/upload — complete operational upload with contract authorization through contracts→bookings chain (shipper_id/transporter_id proof), file validation, SHA-256 hashing, pre-storage dedup check, atomic document+linkage+version creation, orphaned-file cleanup on DB failure + race-condition storage cleanup. (2) GET /api/documents — list endpoint supports linkedEntityType/linkedEntityId filtering plus backward-compatible entityType/entityId aliases, status/documentType pagination. (3) GET /api/documents/[id]/download — signed URL redirect with specific document access verification (creator ownership, admin bypass, shipper/transporter entity-chain check). (4) GET /api/documents/[id]/preview — same authorization as download for preview URLs. (5) DocumentRepository.createWithDuplicateHandling() — concurrency-safe insert returning { document, isNew }; used alongside partial unique index for atomic dedup. (6) ShipmentDocumentViewer UI — fixed API parameter mismatch (entityType→linkedEntityType). AUTHORIZATION MODEL: SHIPPER may associate documents only with contracts in their booking scope; TRANSPORTER same; ADMIN/SUPER_ADMIN full access; MODERATOR read-only; DRIVER/DISPATCHER restricted. CONCURRENCY: pre-check dedup BEFORE storage + partial unique index on (file_hash, linked_entity_type, linked_entity_id) WHERE is_deleted = false (migration 028, requires live application via Supabase dashboard or CLI) + createWithDuplicateHandling() catches any race. STORAGE CONSISTENCY: storage path tracked; on DB failure after storage: deleteFile() cleans up orphan; on race-loss (isNew=false): caller's storage object also cleaned up. TESTS: 1,797 passing across 91 files. TS: 0 errors. BUILD: compiles successfully (pre-existing /forgot-password prerender error unrelated). BLOCKED ON LIVE GATES: (a) Migration 028 unapplied on live database — verified via REST API index check returned empty result; requires manual SQL application via Supabase Dashboard. (b) Live E2E testing impossible from this development environment due to DNS/network restrictions blocking direct PostgreSQL connections to db.cvdggspiudmudbbdkjrc.supabase.co; application IS deployed on Vercel where runtime CAN reach Supabase. After deployment verify at https://cvdggspiudmudbbdkjrc.supabase.co/rest/v1/. Deploy to Vercel for production activation. | 2026-09-18 | Governance/Closure |
...
**NONE   All Waves 0-4 complete. Custom-Schema Data API Remediation (HAO-CUSTOM-SCHEMA-REMED-CLOSE) completed, verified, and closed (2026-09-18). C7-004 Document→Contract Auto-Association IMPLEMENTED BUT AWAITING LIVE VERIFICATION (2026-09-18): complete operational upload with contract authorization through contracts→bookings chain, pre-storage dedup check, partial unique index migration 028 for atomic concurrency safety (UNAPPLIED on live database per REST API index check — requires manual SQL application via Supabase Dashboard SQL Editor), storage/DB consistency with orphan cleanup + race-condition storage cleanup, download/preview with specific document access verification. Live E2E testing impossible from this environment due to DNS/network restrictions blocking TCP DB access; application IS deployed on Vercel (https://cvdggspiudmudbbdkjrc.supabase.co). Tests: 1,797 passing across 91 files. TypeScript: 0 errors. Build: compiles successfully (pre-existing /forgot-password prerender error unrelated). C7 tracking/security work closed through d9d2c52; governance normalized. C7-005 remains NOT AUTHORIZED. Driver/PWA deferred.**
---
