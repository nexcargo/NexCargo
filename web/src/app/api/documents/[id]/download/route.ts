 ...
 * - SHIPPER: can access if linked_entity_type=booking AND user is booking's shipper
 *            OR if linked_entity_type=contract AND user is contract's booking's shipper
 * - TRANSPORTER: same as SHIPPER but checks transporter_id
 * - Other roles: rely on RBAC-only (checked in route handler)
 */
async function verifyDocumentAccess(
  ...
  if (entityType === 'contract') {
    // Check through contracts → bookings chain
    if (userRole === 'SHIPPER') {
      const result = await adminClient
        .schema('logistics_schema')
        .from('contracts')
  ...
  // User documents (linked_entity_type='user') - owner check already above

  return false;
}

/** GET /api/documents/[id]/download — Redirect to signed storage URL for download */
